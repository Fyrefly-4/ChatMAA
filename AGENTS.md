...
以下规则适用于 feature 分支上的开发提交，并且仅当当前任务需要创建提交时执行。`main` 上的提交通常来自 squash merge，不能用于推断 feature 分支的提交拆分方式。

1. 每个提交只解决一个明确问题，标题准确描述能力增量。
2. 按依赖顺序提交，先提交底层契约和执行能力，再提交上层编排及入口。
3. 实现、对应测试和必要配置进入同一提交；依赖变更产生的 lockfile 与依赖声明一起提交。
4. 仅当内容能够独立验证或回滚时拆分提交，不拆散强耦合代码。
5. 原型的纯迁移或复用与新增行为尽量分开提交。
6. 文档跟随其描述的能力更新；总览型架构文档可以在提交序列末尾统一更新。
7. 完成前逐个检查提交，确保提交能够说明其动机、建立的边界、验证方式和撤销影响，并使整个提交序列按顺序体现实现过程。
