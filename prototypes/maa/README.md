...
后续 HTTP 合并实机验证也已完成：第一局成功，第二局启动后通过 HTTP 停止，两端证据一致、进程退出，用户确认能手动接管。本轮与之前三次分开统计；第二局停止后的结果不补计。模块图、时间线及限制见 [HTTP-MERGE.md](HTTP-MERGE.md)。不追加实机调用。
...
目前 9 项离线检查通过，包括真实单局回调缺少 `cur_times` 的回归用例。DPI 修复后主界面原生识别、单局及两局连续执行、正常收尾已有实机证据和用户现场确认；HTTP 合并链路及第二局启动阶段的主动停止也已有独立实机证据。资源不足、战斗失败、其他战斗阶段和故障强制退出仍未实测。替身实验见 [lifecycle/REPORT.md](../lifecycle/REPORT.md)，不能直接视为实机保证。
