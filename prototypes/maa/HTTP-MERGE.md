...
当前安排：用户选择「暂缓实机，先审阅合并设计」。保留入口供审阅，不运行主界面探针或战斗；后续是否实测再由用户决定。
...
- **仍缺证据**：工作线程替身已证明其等待时 HTTP 可响应，但真实 MaaCore 调用是否具有同样表现、战斗中停止的返回顺序及等待时间，均需实测。两个进程同时失去响应的监督缺口仍存在，合并没有增加兜底承诺。
...
现有 8 项检查分别覆盖显式停止、失联停止和重启补读；它们没有断言运行中正常关闭应用后，两端已经完成最后一次结果交接。拟议的实机驱动也是先等到显式停止结果，再关闭应用，因此即使该轮通过，也不能覆盖此项。

建议落实既有 ADR-0001 的正常退出要求：拒绝新任务后，在有限期限内保留必要的续期与结果查询，先请求停止并把最终证据提交到业务库，再结束 Python；发生超时或存储故障时保留未知部分，继续遵循已接受的退出兜底。应增加离线检查，验证「执行中关闭应用 → Python 停止 → 最终证据到达 TS → 两端退出」及交接失败后的诚实状态。该项是实现与验证待办，当前仅记录审阅意见，没有修改运行逻辑或启动新实验。
...
本轮结果：8/8 合并检查通过（7 项 HTTP / 进程实验和 1 项确定性投影回归）；原型 B 25/25 回归通过；交接驱动的离线演练通过。最后一轮各实验均确认自有进程退出。MAA 参数与计数的原有 9 项单元检查、TS 类型检查及管理员入口语法检查通过。未运行任何新的实机操作。
...
**尚未执行，须由用户另行确认。** 建议最多开战两次 `1-7`：第一局正常完成，用于核对完成量从 MaaCore 经 HTTP 到业务库的全过程；观察到第二局开战且第一局成功后，经 TS 的 HTTP 停止入口请求停止。全程不吃药、不碎石、不主动制造战斗失败、不强杀游戏。
...
经确认后，在游戏主界面、官方 MAA 任务已停止时，管理员 PowerShell 使用以下一次性入口：
...
本轮待测配置为 200 ms 查询与续期、10 s 租约、2 s 单次 HTTP 超时和 20 s 退出停止期限；这些仅是为 native 调用留出收尾时间的实验设置，尚无实机性能或可靠性结论。
