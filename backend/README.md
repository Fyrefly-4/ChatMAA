...
**日常只需一个终端：准备游戏 → 明确范围 → 运行向导 → 现场确认 → 查看报告。** 最近核对：2026-09-18；正式入口基线 `58068ce`，当前本地向导已做离线检查，**完整实机验收尚未完成**。
...
最近核对：2026-09-18，代码基线 `58068ce`。本章按正式入口代码编写，**尚未用新工程完成实机演练**；阅读或更新手册不改变这一验证状态。操作说明在本章维护，模块职责和实现状态见[工程说明](../docs/engineering/architecture.md)，识别与锁的机制见 [Adapter 说明](../adapter/maa/README.md)。
