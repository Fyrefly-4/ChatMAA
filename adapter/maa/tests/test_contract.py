...
class ContractTest(unittest.TestCase):
    ...
    def test_real_single_battle_without_cur_times(self):
        events = json.loads((Path(__file__).resolve().parents[1] / "fixtures/verified-first-battle.json").read_text(encoding="utf-8"))["events"]
    ...
    def test_missing_cycle_evidence_or_conflicting_count_remains_unknown(self):
        source = json.loads((Path(__file__).resolve().parents[1] / "fixtures/verified-first-battle.json").read_text(encoding="utf-8"))["events"]
