...
2026-09-20 D2 开发补充（基于 `3183e07`）：Adapter 增加显式 MuMu/ADB 连接、库存扫描与材料目标操作，Backend 复用原任务服务接收 v2 确定参数并保存独立结果投影；进程、HTTP、双库与模型边界不变。当前只有离线及 native 替身证据，MuMu 实机验收未进行，D2 尚未完成。代码入口、契约与限制见 [D2 执行契约](d2-adapter.md)。Web 仅补充新任务类型的兼容展示，完整方案与对话仍属后续阶段。
