...
退出交接分为两步：宿主开始退出即调用 `beginShutdown`，封锁业务变更和后续处理结果接纳，并取消消费者；执行端停止与交接期间继续保存确定性任务事实、结果及会话摘要，不再生成或派发新的后续理解工作。最终 `close` 等待消费者收尾后解除同步观察，再关闭数据库。不能提前停止全部业务投影而丢失最终成果。
