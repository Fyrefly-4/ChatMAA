...
正式回放另覆盖执行中不可比较目标的调整：Backend 拒绝错误的总目标后，工具仍提交带关联的停止，原任务不恢复且不建立替代执行。外层操作关联事务回滚时，后台事件不会提前领取或启动；派发推迟到事务结束后。

PR #31 调整原子性修正：有效 `adjust_task` 直接复用 `BusinessService.adjust()`，在同一事务中保存新需求、停止预留和 Runtime 关联，提交后仅 dispatch 一次。只有明确的 `incomparable_total_goal` 拒绝才转为带关联的停止，操作结果保存拒绝原因与停止反馈，不创建替代需求。两条路径的关联写入失败都不发送停止；不是遇到任意错误都停止。新增四项 SDK 工具回归覆盖发送次数、关联失败回滚与重复读取不重发。

本轮审查修正后，Backend 类型检查及全量 144 项通过（含正式回放）。未重跑真实模型或游戏；上文真实模型证据仍属于修正前基线，本轮新增证据针对确定性调度与事务边界。
