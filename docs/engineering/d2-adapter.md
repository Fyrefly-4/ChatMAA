...
2026-09-20；实现基于 `3183e07`。状态：D2 开发中的离线实现，MuMu 实机验收尚未进行，不能据此宣布阶段完成。范围见 [D2 #21](https://github.com/Fyrefly-4/ChatMAA/issues/21)，业务含义沿用[协作约定](mvp-collaboration.md)。
...
扫描的正常后置核对包含有限返回：识别仓库标签后，只允许匹配 Return 模板点击一次，再识别首页。它不调用 Fight，也不是通用返回循环；失败不放行。显式历史 recheck／takeover 仍只识别，不新增导航。仓库返回路径只有静态与 native 边界替身检查，尚待 MuMu 实机核对。
...
尚需固定 MuMu／Android／游戏／Core／资源组合，验证扫描可靠性与缺失项、仓库返回、成功次数、材料自停与超额、普通停止和掉落异常延迟、无药无石及连续任务衔接。D2 完成仍依赖这些关键实机事实。
